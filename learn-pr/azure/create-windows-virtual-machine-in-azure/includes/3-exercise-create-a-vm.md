...
![Screenshot mit dem Erstellen eines virtuellen Computers im Azure-Portal erleben.](../media/3-azure-portal-create-vm.png)
...
1. Für **Ressourcengruppe**Option **vorhandene** , und wählen Sie <rgn>[Ressourcengruppennamen Sandkasten]</rgn>.
...
1. In der **INSTANZDETAILS** Geben Sie einen Namen für Ihren virtuellen Computer, z. B. **Test-vp-vm2** (für Test Video Prozessor VM #2).
...
1. In der **ADMINISTRATORKONTO** legen die **Benutzername** Feld mit einem Benutzernamen, die Sie verwenden, um mit dem virtuellen Computer anmelden.
...
1. In der **Regeln für eingehende Ports** Abschnitt, öffnen Sie die Liste, und wählen Sie _lassen Sie die ausgewählten Ports_. Da es sich dabei um einen virtuellen Windows-Computer handelt, soll die Möglichkeit bestehen, mithilfe von RDP auf den Desktop zuzugreifen. Scrollen Sie ggf. durch die Liste, bis Sie RDP (3389) finden, und wählen Sie diese Option aus. In der Benutzeroberfläche wird der Hinweis angezeigt, dass auch nach dem Erstellen des virtuellen Computers die Netzwerkports geändert werden können.

    ![Screenshot der Dropdown-Liste für das Öffnen des Ports für RDP-Zugriff auf die Windows-VM angezeigt.](../media/3-open-ports.png)
...
1. Klicken Sie auf **Weiter**, um zum Abschnitt „Datenträger“ zu navigieren.
    ...
    ![Screenshot der im Abschnitt Datenträger konfigurieren für den virtuellen Computer.](../media/3-configure-disks.png)
...
1. Klicken Sie auf den Link **Create and attach a new disk** (Neuen Datenträger erstellen und anfügen) im Abschnitt **DATA DISKS** (DATENTRÄGER).
    ...
    ![Screenshot mit den neuen virtuellen Computer Datenträger Dialogfeld zur tresorerstellung im Portal.](../media/3-add-data-disk.png)
...
1. Es sollte nun ein neuer Datenträger in der ersten Zeile angezeigt werden.
    ...
    ![Screenshot, der den neu hinzugefügten Datenträger auf dem virtuellen Computer anzeigt.](../media/3-new-disk.png)
...
1. Ändern Sie die Standardbereiche, um den IP-Adressraum `172.xxx` zu verwenden. Klicken Sie auf **erstellen Sie eine neue** unterhalb des virtuellen Netzwerks.
