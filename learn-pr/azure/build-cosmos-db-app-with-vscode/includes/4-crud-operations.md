...
Nach dem Herstellen der Verbindung mit Azure Cosmos DB besteht der nächste Schritt darin, die in der Datenbank gespeicherten Dokumente zu erstellen, zu lesen, zu ersetzen und zu löschen. In dieser Einheit erstellen Sie Benutzerdokumente in Ihrer WebCustomer-Sammlung. Anschließend rufen Sie diese anhand der ID ab, ersetzen sie und löschen sie.

## <a name="working-with-documents-programmatically"></a>Programmgesteuertes Arbeiten mit Dokumenten

Daten werden in Azure Cosmos DB als JSON-Dokumente gespeichert. [Dokumente](https://docs.microsoft.com/azure/cosmos-db/sql-api-resources#documents) können im Portal erstellt, abgerufen, ersetzt oder gelöscht werden, und zwar wie im vorherigen Modul oder wie in diesem Modul beschrieben programmgesteuert. Azure Cosmos DB bietet clientseitige SDKs für .NET, .NET Core, Java, Node.js und Python, die diese Vorgänge unterstützen. In diesem Modul wird das .NET Core SDK verwendet, um CRUD-Vorgänge (Create, Retrieve, Update, Delete – Erstellen, Abrufen, Aktualisieren, Löschen) für die in Azure Cosmos DB gespeicherten NoSQL-Daten auszuführen.

Die wichtigsten Vorgänge für Azure Cosmos DB-Dokumente sind Teil der Klasse [DocumentClient](https://docs.microsoft.com/dotnet/api/microsoft.azure.documents.client.documentclient?view=azure-dotnet):
...
1. Kopieren Sie jetzt die Aufgabe **CreateUserDocumentIfNotExists**, und fügen Sie diese unter der Funktion **WriteToConsoleAndPromptToContinue** am Ende der Datei „Program.cs“ ein.
...
1. Kehren Sie dann zur Methode **BasicOperations** zurück, und fügen Sie Folgendes am Ende der Methode hinzu.
...
1. Geben Sie im integrierten Terminal erneut den folgenden Befehl zum Ausführen des Programms ein.

    ```csharp
    dotnet run
    ```

    Im Terminal wird jeweils eine Ausgabe angezeigt, während die Anwendung die einzelnen neuen Benutzerdokumente erstellt. Drücken Sie eine beliebige Taste, um die Anwendung zu beenden.
...
1. Um Dokumente aus der Datenbank zu lesen, kopieren Sie den folgenden Code, und fügen Sie ihn nach der WriteToConsoleAndPromptToContinue-Methode in die Datei „Program.cs“ ein.
...
1. Kopieren Sie den folgenden Code, und fügen Sie ihn am Ende der Methode **BasicOperations** nach der Zeile `await this.CreateUserDocumentIfNotExists("Users", "WebCustomers", nelapin);` ein.

    ```csharp
    await this.ReadUserDocument("Users", "WebCustomers", yanhe);
    ```

1. Geben Sie im integrierten Terminal den folgenden Befehl zum Ausführen des Programms ein.
...
1. Kopieren Sie die Methode **ReplaceFamilyDocument**, und fügen Sie sie nach der ReadUserDocument-Methode in die Datei „Program.cs“ ein.
...
1. Kopieren Sie die Methode **DeleteUserDocument**, und fügen Sie sie unterhalb der Methode **ReplaceUserDocument** ein.
...
1. Kopieren Sie den folgenden Code, und fügen Sie ihn am Ende der Methode **BasicOperations** ein.
...
1. Führen Sie im integrierten Terminal den folgenden Befehl aus.
    ...
    Im Terminal wird die folgende Ausgabe angezeigt, wobei die Ausgabe „Deleted user 1“ (Benutzer 1 gelöscht) angibt, dass das Dokument gelöscht wurde.
