...
Die Speicherdienstverschlüsselung bietet einen niedrigen Verschlüsselungsschutz für Daten, die auf einen physischen Datenträger geschrieben werden. Wie schützen Sie also die virtuellen Festplatten (VHDs) von virtuellen Computern (VMs)? Angenommen ein bösartiger Angreifer erhält Zugriff auf Ihr Azure-Abonnement und filtert die VHDs Ihrer virtuellen Computer: Wie stellen Sie dann sicher, dass er nicht auf die VHD-Daten zugreifen kann?
...
Lamna Healthcare hat den Identifizierungs- und Klassifizierungsprozess für alle gespeicherten Daten durchlaufen. Die Organisation hat diese Klassifizierungen mit den gesetzlichen und geschäftlichen Anforderungen abgeglichen und festgestellt, dass sie weitaus mehr Daten hat, die verschlüsselt werden müssen. Es wurden alle VMs verschlüsselt, die vertrauliche Daten beinhalten. Außerdem verschlüsselt Lamna Healthcare alle vertraulichen Patienteninformationen, die in Blob Storage gespeichert werden. TDE ist für alle Datenbanken aktiviert, sodass die relationalen Datenbanken die Verschlüsselungsanforderungen unabhängig von der Klassifizierung erfüllen. Lamna Healthcare hat die gesamte Organisation berücksichtigt und verwendet Key Vault, um alle Zertifikate und Anmeldeinformationen zu speichern, die Anwendungen für den Betrieb benötigen.
