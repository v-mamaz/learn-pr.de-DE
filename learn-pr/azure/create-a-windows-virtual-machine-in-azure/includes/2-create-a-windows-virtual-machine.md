...
Es gibt verschiedene Möglichkeiten, VMs in Azure zu definieren und bereitzustellen: das Azure-Portal, ein Skript (unter Verwendung von PowerShell oder der Azure CLI) oder eine Azure Resource Manager-Vorlage. Sie müssen für jede dieser Optionen mehrere Informationen angeben. Diese werden später noch thematisiert.
...
![Azure Marketplace-VMs](../media-drafts/2-marketplace-vm-choices.png)

## <a name="resources-used-in-a-windows-vm"></a>Auf einer Windows-VM verwendete Ressourcen

Wenn Sie eine Windows-VM in Azure erstellen, erstellen Sie gleichzeitig auch Ressourcen, um diese zu hosten. Diese Ressourcen arbeiten zusammen, um eine VM zu erstellen und das Windows-Betriebssystem auszuführen. Sie sind entweder bereits vorhanden (und werden während der Erstellung der VM ausgewählt), oder sie werden gemeinsam mit der VM erstellt. Folgende Ressourcen müssen vorhanden sein:
...
- Ein virtuelles Netzwerk (VNet), über das die VM mit anderen Azure-Diensten oder Ihrer eigenen lokalen Hardware verbunden wird.
- Eine Netzwerkschnittstelle, die zur Kommunikation mit dem VNet dient.
- Eine öffentliche IP-Adresse, damit Sie auf die VM zugreifen können. Dies ist optional.

Wie bei anderen Azure-Diensten auch benötigen Sie eine **Ressourcengruppe**, in der die VM gespeichert wird. Sie sollten diese Ressourcen am besten zu Verwaltungszwecken in einer Gruppe zusammenfassen. Bei der Erstellung einer neuen VM können Sie entweder eine vorhandene Ressourcengruppe verwenden oder eine neue erstellen.
...
Ein virtueller Computer weist wie ein physischer Computer eine bestimmte Menge an Arbeitsspeicher und CPU-Leistung auf. Azure bietet eine Reihe von VMs mit unterschiedlichen Größen zu unterschiedlichen Preisen an. Die Größe, die Sie auswählen, bestimmt die Verarbeitungsleistung, den Speicher und die maximale Speicherkapazität Ihrer VM.

> [!WARNING]
> Es sind Grenzwerte für die einzelnen Abonnements festgelegt, die Einfluss auf die Erstellung der VM haben können. Standardmäßig dürfen Ihre VMs innerhalb einer Region insgesamt nicht mehr als 20 virtuelle _Kerne_ aufweisen. Sie können die VMs entweder auf mehrere Regionen aufteilen oder einen [Onlineantrag](https://docs.microsoft.com/en-us/azure/azure-supportability/resource-manager-core-quotas-request) stellen, damit die Grenzwerte erhöht werden.

Die VM-Größen werden in Kategorien unterteilt, angefangen bei der B-Serie für grundlegende Tests, bis hin zur H-Serie für große Computingtasks. Wählen Sie die Größe der VM anhand der Workload aus, die Sie ausführen möchten. Sie können die Größe einer VM zwar ändern, nachdem Sie sie erstellt haben, aber Sie müssen diese dafür beenden. Daher ist es besser, wenn Sie direkt zu Beginn die richtige Größe auswählen.
...
Sie können Daten zusammen mit dem Betriebssystem auf dem Laufwerk C: speichern, aber ein besserer Ansatz ist es, dedizierte _Datenträger_ zu erstellen. Sie können weitere Datenträger erstellen und an den virtuellen Computer anfügen. Jeder Datenträger kann bis zu 4.095 GB Daten enthalten. Die maximale Speicherkapazität wird durch die Größe des virtuellen Computers bestimmt, den Sie auswählen.

> [!NOTE]
> Eine interessante Möglichkeit stellt die Erstellung eines VHD-Images von einem echten Datenträger dar. Dies ermöglicht die einfache Migration _vorhandener_ Informationen von einem lokalen Computer in die Cloud.

### <a name="unmanaged-vs-managed-disks"></a>Nicht verwaltete Datenträger und verwaltete Datenträger im Vergleich
...
- **Unterstützung von Momentaufnahmen**: Momentaufnahmen können verwendet werden, um eine schreibgeschützte Kopie einer VHD zu erstellen. Sie müssen den übergeordneten virtuellen Computer zwar herunterfahren, aber das Erstellen der Momentaufnahme dauert nur wenige Sekunden. Wenn dieser Vorgang abgeschlossen ist, können Sie den virtuellen Computer aktivieren und die Momentaufnahme verwenden, um den virtuellen Computer zu duplizieren, damit Sie ein Produktionsproblem beheben oder ein Rollback des virtuellen Computers zu dem Zeitpunkt erstellen können, zu dem die Momentaufnahme erstellt wurde.
- **Sicherungsunterstützung:** Verwalteter Datenträger können mithilfe von Azure Backup automatisch zur Notfallwiederherstellung in verschiedenen Regionen gesichert werden, sodass die VM nicht beeinträchtigt wird.
...
Es ist zwar einfach, Azure das Netzwerk zusammen mit dem virtuellen Computer erstellen zu lassen, aber es ist wahrscheinlich für die meisten Szenarios nicht ideal. Es ist besser, Ihre Netzwerkanforderungen _im Voraus_ für alle Komponenten Ihrer Architektur zu planen und die VNet-Struktur separat zu erstellen. Erstellen Sie dann die virtuellen Computer, und platzieren Sie sie in den bereits erstellten VNets.
