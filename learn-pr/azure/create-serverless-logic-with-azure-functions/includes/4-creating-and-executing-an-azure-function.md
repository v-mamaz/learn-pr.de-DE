...
Bindungen sind eine deklarative Möglichkeit, Daten und Dienste mit Ihrer Funktion zu verbinden. Bindungen wissen, wie wenden Sie sich an verschiedene Dienste, sodass Sie nicht zum Schreiben von Code in Ihrer Funktion, um eine Verbindung mit Datenquellen herstellen und Verwalten von Verbindungen. Die Plattform übernimmt diese Komplexität für Sie als Teil des Codes für die Bindung. Jede Bindung hat eine Richtung. Ihr Code liest Daten aus *Eingabe*bindungen und schreibt Daten in *Ausgabe*bindungen. Jede Funktion kann null oder mehr Bindungen haben, um die von der Funktion verarbeiteten Ein- und Ausgabedaten zu verwalten.

Ein Trigger ist eine besondere Art von eingabebindung, die zusätzliche die Möglichkeit der Ausführung initiiert hat.
...
Bindungen können im Azure-Portal definiert werden und werden als JSON-Dateien, die Sie auch direkt bearbeiten können, gespeichert. Der folgende JSON-Code ist eine Beispieldefinition eines Triggers und einer Bindung für eine Funktion.
...
![Screenshot des Azure-Portal das Blatt "Funktionen-Editor", einschließlich der erweiterten Dateien im Menü Ansicht, mit der ausgewählten "HttpTriggerJS1"-Funktion in unserer app Service-Navigation und Dateien im Menü Ansicht mit hervorgehoben.](../media/4-file-navigation.png)

Wie Sie im obigen Screenshot sehen können, müssen Sie ein Flyout-Menü vorhanden ist, auf der rechten Seite, die eine Registerkarte, um enthält **Ansichtsdateien**. Wenn Sie diese Registerkarte auswählen, wird die Dateistruktur angezeigt, aus der sich Ihre Funktion zusammensetzt.
...
Sie können eine Funktion starten, indem Sie den konfigurierten Trigger manuell auslösen. Z. B. bei Verwendung ein HTTP-Triggers - können ein Tool wie Postman oder cURL eine HTTP-Anforderung an die Funktion Endpunkt-URL, initiiert werden, die aus der Definition der HTTP-Trigger verfügbar ist (**Funktions-URL abrufen**).

### <a name="testing-in-the-azure-portal"></a>Testen im Azure-Portal

Das Azure-Portal bietet auch eine praktische Möglichkeit, Ihre Funktionen zu testen. Auf der rechten Seite des Codefensters finden Sie ein Flyoutmenü mit Registerkartennavigation. Dieses Menü enthält ein Element **Test**. Das Erweitern des Menüs und das Auswählen dieser Registerkarte ist eine weitere Möglichkeit, Ihre Funktion auszuführen und das Ergebnis anzuzeigen. Wenn Sie in diesem Testfenster auf **Ausführen** klicken, werden die Ergebnisse zusammen mit einem Statuscode im Ausgabefenster angezeigt.
...
![Screenshot des Azure-Portal mit einer HTTP-Funktion Blatt "überwachen" mit der mehrere Funktionsergebnisse und ihren entsprechenden HTTP-Statuscodes mit dem Modul Menüelement, das der Funktion hervorgehoben.](../media/4-monitor-function.png)

## <a name="streaming-log-window"></a>Fenster mit dem Streamingprotokoll

Zu Debugzwecken können Sie Ihrer Funktion im Azure-Portal auch Protokollierungsanweisungen hinzufügen. Die aufgerufenen Methoden für jede Sprache werden ein "Protokollierung"-Objekt übergeben, die verwendet werden kann, um Informationen an das Protokollfenster, befindet sich in einem befindet sich am unteren Rand im Code-Fenster im Registerkartenformat Flyout-Menü zu protokollieren.
